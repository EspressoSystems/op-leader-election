...
type DataAvailabilitySource interface {
	OpenData(ctx context.Context, id eth.BlockID, batcherAddr common.Address, batcherHashVersion uint8) DataIter
}
...
func (l1r *L1Retrieval) NextData(ctx context.Context) ([]byte, error) {
	if l1r.datas == nil {
		...
		cnf := l1r.prev.SystemConfig()
		l1r.datas = l1r.dataSrc.OpenData(ctx, next.ID(), cnf.BatcherAddr, cnf.BatcherHashVersion)
	}
...
func (l1r *L1Retrieval) Reset(ctx context.Context, base eth.L1BlockRef, sysCfg eth.SystemConfig) error {
	l1r.datas = l1r.dataSrc.OpenData(ctx, base.ID(), sysCfg.BatcherAddr, sysCfg.BatcherHashVersion)
