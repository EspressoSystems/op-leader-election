...
interface ILeaderElectionBatchInbox {
    ...
    /// @param leaderId identifier of the leader. If the null address  0x0000000000000000000000000000000000000000 is
    /// passed, the address of the caller is considered.
    /// @param blockNumber block number for testing whether some participant is the leader. If blockNumber=0 then the
    ...
    ///         Example if the current block is 50, and the caller is the leader for blocks 50, 51, 53, 57, then the
    /// integer will represent the bitmap array [1,1,0,1,0,0,0,1], encoded as 139 in little endian.
    ///         Additionally we provide another uint8 value to specify that the 0s after some position mean "not defined
    /// yet" instead of "not a leader". In this example this value is N=8.
